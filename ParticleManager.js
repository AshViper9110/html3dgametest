class ParticleManager {
  ...
  _geo(key, factory) {
    ...
    return this._geoCache[key] = SHARED ? SHARED.geo(key, factory) : factory();
  }
  ...
  _addParticle(color, pos, vel, life, size) {
    const key = (size && Math.abs(size - 0.1) > 0.001)
      ? 'pm_particle_' + size.toFixed(3)
      : 'pm_particle_std';
    const geo = this._geo(key, () => new THREE.SphereGeometry(size || 0.1, 4, 4));
    return this._createParticle(geo, color, pos, vel, life, 'particle', true, 0);
  }

  _addSpark(color, pos, vel, life) {
    const geo = this._geo('pm_spark', () => new THREE.BoxGeometry(0.04, 0.04, 0.12));
    return this._createParticle(geo, color, pos, vel, life, 'spark', true, 0);
  }

  _addSmokeParticle(color, pos, vel, life) {
    const geo = this._geo('pm_smoke', () => new THREE.SphereGeometry(0.3, 6, 6));
    const e = this._createParticle(geo, color, pos, vel, life, 'smoke', false, 0.5 + Math.random() * 0.5);
    return e;
  }

  _addDebris(pos, vel, life) {
    const geo = this._geo('pm_debris', () => new THREE.BoxGeometry(0.08, 0.06, 0.08));
    const e = this._createParticle(geo, 0x888888, pos, vel, life, 'debris', true, 0);
    if (e) {
  ...
  _addSpeedLine(color, pos, vel, life) {
    const geo = this._geo('pm_speedline', () => new THREE.BoxGeometry(0.02, 0.02, 0.5));
    const e = this._createParticle(geo, color, pos, vel, life, 'speedline', false, 0);
    if (e) {
      _v3c.copy(pos).add(vel);
      e.mesh.lookAt(_v3c);
    }
    return e;
  }
  ...
  spawnExplosionParticles(pos, color, count) {
    ...
    for (let i = 0; i < count; i++) {
      ...
      this._addParticle(color || 0xff4400, pos, _v3c, 0.4 + Math.random() * 0.4, 0.2 + Math.random() * 0.3);
    }
  }
  ...
  spawnSparks(pos, color, count) {
    ...
    for (let i = 0; i < count; i++) {
      ...
      this._addSpark(color || 0xffff00, pos, _v3c, 0.15 + Math.random() * 0.15);
    }
  }
  ...
  spawnSmoke(pos, color, count) {
    ...
    for (let i = 0; i < count; i++) {
      ...
      this._addSmokeParticle(color || 0x444444, pos, _v3c, 0.6 + Math.random() * 0.4);
    }
  }

  spawnDebris(pos, count) {
    count = Math.min(count || 3, 4);
    for (let i = 0; i < count; i++) {
      const angle = Math.random() * Math.PI * 2;
      ...
      this._addDebris(pos, _v3c, 0.3 + Math.random() * 0.3);
    }
  }
  ...
  spawnRingParticles(pos, color, count, radius) {
    ...
    for (let i = 0; i < count; i++) {
      ...
      this._addParticle(color || 0xffffff, pos, _v3c, 0.3, 0.15);
    }
  }
  ...
  spawnNeonBurst(pos, color, count) {
    ...
    for (let i = 0; i < count; i++) {
      ...
      this._addParticle(color || 0x00f0ff, pos, _v3c, 0.4 + Math.random() * 0.4, 0.12 + Math.random() * 0.1);
    }
  }
  ...
  spawnSpeedLines(pos, dir, count) {
    ...
    for (let i = 0; i < count; i++) {
      ...
      this._addSpeedLine(0x00f0ff, _v3c, _v3a, 0.12);
    }
  }
  ...
  spawnRespawnParticles(pos, color, count) {
    ...
    for (let i = 0; i < count; i++) {
      ...
      this._addParticle(color || 0x00f0ff, pos, _v3c, 0.5 + Math.random() * 0.4, 0.15 + Math.random() * 0.15);
    }
  }
  ...
  spawnJumpPadParticles(pos, color) {
    for (let i = 0; i < 5; i++) {
      ...
      this._addParticle(color || 0x00f0ff, pos, _v3c, 0.4, 0.12);
    }
  }
  ...
  clear() {
    ...
    for (const key in cache) {
      if (cache[key].dispose) cache[key].dispose();
    }
    this._geoCache = {};
  }
